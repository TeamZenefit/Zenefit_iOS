...
final class ManualViewController: BaseViewController {
    ...
    private let firstOnboardingView = UIImageView(image: UIImage(resource: .manualOne).withRenderingMode(.alwaysOriginal))
    
    private let secondOnboardingView = ManualView(title: "나의 유형과 개인 설정",
                                                  subTitle: "나는 혜택을 잘 챙기고 있는지 유형으로 알려줘요!\n혜택을 잘 챙겨서 친구들을 이겨봐요!",
                                                  image: .init(resource: .manualTwo))
    
    private let thirdOnboardingView = ManualView(title: "수혜중인 정책 관리",
                                                 subTitle: "이미 수혜중이거나 신청을 완료한 정책은\n체크표시를 눌러 따로 모아 보세요!",
                                                 image: .init(resource: .manualThree))

    
    private let fourthOnboardingView = ManualView(title: "찾아주는 나의 정책",
                                                  subTitle: "입력한 개인정보로 신청 가능 여부를 판단해주고\n예상 수혜 금액이 높은 정책을 먼저 추천드려요!",
                                                  image: .init(resource: .manualFour))
    
    private let fifthOnboardingView = ManualView(title: "관심 있는 정책 신청 관리",
                                                 subTitle: "달력에서 신청 시작일과 마감일을 보여드려요\n관심 등록하면 신청일에 Push 알림을 보내드려요!",
                                                 image: .init(resource: .manualFive))
